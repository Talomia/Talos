...
if (initialState.token && !initialState.stats) {
  fetchSupabaseStats(initialState.token).catch((e) => logger.error('Failed to fetch initial Supabase stats:', e));
}
...
export function initializeSupabaseConnection() {
  ...
  if (envToken && !supabaseConnection.get().token) {
    ...
    fetchSupabaseStats(envToken).catch((e) => logger.error('Failed to fetch Supabase stats:', e));
  }
}
...
export async function fetchSupabaseStats(token: string) {
  ...
  try {
    ...
    const data = (await response.json()) as { user?: SupabaseUser | null; stats?: SupabaseStats };
