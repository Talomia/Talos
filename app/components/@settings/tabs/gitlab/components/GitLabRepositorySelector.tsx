...
export function GitLabRepositorySelector({ onClone, className }: GitLabRepositorySelectorProps) {
  ...
  if (!isConnected || !connection) {
    return (
      <div className="text-center p-8">
        <p className="text-ui-textSecondary mb-4">Please connect to GitLab first to browse repositories</p>
    ...
    return (
      <div className="text-center p-8">
        <div className="text-red-500 mb-4">
          ...
          <p className="text-sm text-ui-textSecondary mt-1">{error}</p>
        </div>
  ...
  if (isLoading && !repositories.length) {
    return (
      <div className="flex flex-col items-center justify-center p-8 space-y-4">
        ...
        <p className="text-sm text-ui-textSecondary">Loading repositories...</p>
      </div>
    );
  }
  ...
  if (!repositories.length && !isLoading) {
    return (
      <div className="text-center p-8">
        <div className="i-ph:git-branch w-12 h-12 text-ui-textTertiary mx-auto mb-4" />
        <p className="text-ui-textSecondary mb-4">No repositories found</p>
        <Button variant="outline" onClick={handleRefresh} disabled={isRefreshing}>
          <div className={classNames('i-ph:arrows-clockwise w-4 h-4 mr-2', { 'animate-spin': isRefreshing })} />
          Refresh
        </Button>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-ui-textPrimary">Select Repository to Clone</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="relative flex-1">
          <div className="i-ph:magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ui-textTertiary" />
          <input
            type="text"
            placeholder="Search repositories..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 rounded-lg bg-ui-background-depth-1 border border-ui-borderColor text-ui-textPrimary placeholder-ui-textTertiary focus:outline-none focus:ring-1 focus:ring-ui-borderColorActive"
          />
        </div>

        {/* Sort */}
        <div className="flex items-center gap-2">
          <div className="i-ph:calendar w-4 h-4 text-ui-textTertiary" />
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortOption)}
            className="px-3 py-2 rounded-lg bg-ui-background-depth-1 border border-ui-borderColor text-ui-textPrimary text-sm focus:outline-none focus:ring-1 focus:ring-ui-borderColorActive"
          >
        ...
        {/* Filter */}
        <div className="flex items-center gap-2">
          <div className="i-ph:funnel w-4 h-4 text-ui-textTertiary" />
          <select
            value={filterBy}
      ...
      {currentRepositories.length > 0 ? (
        <>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t border-ui-borderColor">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-ui-textSecondary px-3">
        ...
        <div className="text-center py-8">
          <p className="text-ui-textSecondary">No repositories found matching your search criteria.</p>
        </div>
      )}
