...
const NotificationsTab = () => {
    ...
    switch (level) {
      ...
      default:
        return {
          ...
          bg: 'hover:bg-ui-item-backgroundActive/10',
        };
    }
  };
  ...
  const renderNotificationDetails = (details: NotificationDetails) => {
    if (details.type === 'update') {
      return (
        <div className="flex flex-col gap-2">
          <p className="text-sm text-ui-textSecondary">{details.message}</p>
          <div className="flex flex-col gap-1 text-xs text-ui-textTertiary">
    ...
    return details.message ? <p className="text-sm text-ui-textSecondary">{details.message}</p> : null;
  };
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {filterOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.id}
                  className="group flex items-center px-4 py-2.5 text-sm text-ui-textSecondary hover:bg-purple-500/10 cursor-pointer transition-colors"
        ...
        >
          <span className="i-ph:trash text-lg text-ui-textTertiary group-hover:text-purple-500 transition-colors" />
          Clear All
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {filteredLogs.length === 0 ? (
          ...
          >
            <span className="i-ph:bell-slash text-4xl text-ui-textTertiary" />
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-ui-textPrimary">No Notifications</h3>
              <p className="text-sm text-ui-textTertiary">You're all caught up!</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => {
            const detailType = log.details?.type;
            const style = getNotificationStyle(log.level, typeof detailType === 'string' ? detailType : undefined);
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <h3 className="text-sm font-medium text-ui-textPrimary">{log.message}</h3>
                  ...
                  <time className="shrink-0 text-xs text-ui-textTertiary">
