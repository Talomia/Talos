...
export function exportAsCSV(ctx: LogExportContext): void {
  try {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `event-logs-${new Date().toISOString()}.csv`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
...
export function exportAsPDF(ctx: LogExportContext): void {
  try {
    ...
    doc.text('AI Development Platform', margin, 45);
    yPos = 70;
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      // Add footer text
      doc.text('Generated by AI Platform', margin, doc.internal.pageSize.getHeight() - 10);
    ...
    // Save the PDF
    doc.save(`event-logs-${new Date().toISOString()}.pdf`);
