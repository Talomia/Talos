...
// Provider icon CSS classes using the standard UnoCSS Phosphor icon system.
const PROVIDER_ICONS: Record<string, string> = {
  AmazonBedrock: 'i-ph:cloud',
      ...
      >
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-ui-textPrimary">Cloud Providers</h4>
              <p className="text-sm text-ui-textSecondary">Connect to cloud-based AI models and services</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-sm text-ui-textSecondary">Enable All Cloud</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProviders.map((provider, index) => (
            <motion.div
              key={provider.name}
              className={classNames(
                'rounded-lg border bg-ui-background text-ui-textPrimary shadow-sm',
            ...
            >
              ...
              <div className="flex items-start gap-4 p-4">
                <motion.div
                  className={classNames(
                    ...
                    'bg-ui-background-depth-3 group-hover:bg-ui-background-depth-4',
                    'transition-all duration-200',
                    provider.settings.enabled ? 'text-accent-500' : 'text-ui-textSecondary',
                  )}
                ...
                >
                  <div className={classNames('w-6 h-6', 'transition-transform duration-200', 'group-hover:rotate-12')}>
                    <div
                      className={classNames(PROVIDER_ICONS[provider.name] || 'i-ph:robot', 'w-full h-full')}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-2">
                    <div>
                      <h4 className="text-sm font-medium text-ui-textPrimary group-hover:text-accent-500 transition-colors">
                        {provider.name}
                      </h4>
                      <p className="text-xs text-ui-textSecondary mt-0.5">
                        ...
                        {editingProvider === provider.name ? (
                          <input
                            ...
                            className={classNames(
                              ...
                              'bg-ui-background-depth-3 border border-ui-borderColor',
              ...
              <motion.div
                ...
                animate={{
                  borderColor: provider.settings.enabled ? 'var(--ui-borderColorActive)' : 'transparent',
