...
export const Slider = genericMemo(<T,>({ selected, options, setSelected }: SliderProps<T>) => {
  ...
  return (
    <div className="flex items-center flex-wrap shrink-0 gap-1 bg-ui-background-depth-1 overflow-hidden rounded-full p-1">
  ...
  return (
    <button
      ...
      className={classNames(
        ...
        selected ? 'text-ui-item-contentAccent' : 'text-ui-item-contentDefault hover:text-ui-item-contentActive',
      )}
    >
      ...
      {selected && (
        <motion.span
          ...
          className="absolute inset-0 z-0 bg-ui-item-backgroundAccent rounded-full"
        ></motion.span>
      )}
    </button>
  );
