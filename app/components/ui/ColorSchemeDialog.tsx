  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-ui-textPrimary flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-ui-item-contentAccent"></div>
          Color Palette
        </h3>
        <button
          onClick={handleReset}
          className="text-sm bg-transparent hover:bg-ui-bg-depth-2 text-ui-textSecondary hover:text-ui-textPrimary rounded-lg flex items-center gap-2 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {paletteRoles.map((role) => (
          <div
            key={role.key}
            className="group flex items-center gap-4 p-4 rounded-xl bg-ui-bg-depth-3 hover:bg-ui-bg-depth-2 border border-transparent hover:border-ui-borderColor transition-all duration-200"
          >
            <div className="relative flex-shrink-0">
              <div
                className="w-12 h-12 rounded-xl shadow-md cursor-pointer transition-all duration-200 hover:scale-110 ring-2 ring-transparent hover:ring-ui-borderColorActive"
              ...
              <input
                ...
                tabIndex={-1}
              />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-ui-bg-depth-1 rounded-full flex items-center justify-center shadow-sm">
                <span className="i-ph:pencil-simple text-xs text-ui-textSecondary" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-ui-textPrimary transition-colors">{role.label}</div>
              <div className="text-sm text-ui-textSecondary line-clamp-2 leading-relaxed">{role.description}</div>
              <div className="text-xs text-ui-textTertiary font-mono mt-1 px-2 py-1 bg-ui-bg-depth-1 rounded-md inline-block">
                {palette[role.key]}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {designFonts.map((f) => (
          <button
            ...
            className={`group p-4 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ui-borderColorActive ${
              font.includes(f.key)
                ? 'bg-ui-item-backgroundAccent border-ui-borderColorActive shadow-lg'
                : 'bg-ui-background-depth-3 border-ui-borderColor hover:border-ui-borderColorActive hover:bg-ui-bg-depth-2'
            }`}
          >
            <div className="text-center space-y-2">
              <div
                className={`text-2xl font-medium transition-colors ${
                  font.includes(f.key) ? 'text-ui-item-contentAccent' : 'text-ui-textPrimary'
                }`}
              ...
              <div
                className={`text-sm font-medium transition-colors ${
                  font.includes(f.key) ? 'text-ui-item-contentAccent' : 'text-ui-textSecondary'
                }`}
              >
                {f.label}
              </div>
              {font.includes(f.key) && (
                <div className="w-6 h-6 mx-auto bg-ui-item-contentAccent rounded-full flex items-center justify-center">
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                ...
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 bg-ui-background-depth-3 text-ui-item-textSecondary ${
                  f.key === 'rounded'
                    ? isSelected
                      ? 'rounded-3xl'
                      : 'rounded-xl'
                  ...
                  f.key === 'border'
                    ? isSelected
                      ? 'border-3 border-ui-borderColorActive bg-ui-item-backgroundAccent text-ui-item-contentAccent'
                      : 'border-2 border-ui-borderColor hover:border-ui-borderColorActive text-ui-textSecondary'
                    : f.key === 'gradient'
                      ? ''
                      : isSelected
                        ? 'bg-ui-item-backgroundAccent text-ui-item-contentAccent shadow-lg'
                        : 'bg-ui-bg-depth-3 hover:bg-ui-bg-depth-2 text-ui-textSecondary hover:text-ui-textPrimary'
                ...
                style={{
                  ...(f.key === 'gradient' && {
                    background: isSelected
                      ...
                      : 'var(--ui-bg-depth-3)',
                    color: isSelected ? 'white' : 'var(--ui-textSecondary)',
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-ui-bg-depth-1 bg-opacity-20">
                    {f.key === 'rounded' && (
                      <div
    ...
    <div>
      ...
      <DialogRoot open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            ...
            {/* Navigation Tabs */}
            <div className="flex gap-1 p-1 bg-ui-bg-depth-3 rounded-xl">
              {(
                [
                  { key: 'colors' as const, label: 'Colors', icon: 'i-ph:palette' },
                  { key: 'typography' as const, label: 'Typography', icon: 'i-ph:text-aa' },
                  { key: 'features' as const, label: 'Features', icon: 'i-ph:magic-wand' },
                ] as const
              ).map((tab) => (
                <button
                  key={tab.key}
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-200 ${
                    activeSection === tab.key
                      ? 'bg-ui-background-depth-3 text-ui-textPrimary shadow-md'
                      : 'bg-ui-background-depth-2 text-ui-textSecondary hover:text-ui-textPrimary hover:bg-ui-bg-depth-2'
                  }`}
                >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  variant="ghost"
                  onClick={handleSave}
                  className="bg-ui-button-primary-background hover:bg-ui-button-primary-backgroundHover text-ui-button-primary-text"
                >
                  Save Changes
                </Button>
              </div>
      ...
      <style>{`
        .custom-scrollbar {
          ...
          scrollbar-color: var(--ui-textTertiary) transparent;
        }
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background-color: var(--ui-textTertiary);
          border-radius: 3px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background-color: var(--ui-textSecondary);
        }
        .line-clamp-2 {
          display: -webkit-box;
          -webkit-line-clamp: 2;
