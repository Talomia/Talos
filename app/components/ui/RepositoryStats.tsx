...
export function RepositoryStats({ stats, className, compact = false }: RepositoryStatsProps) {
  ...
  return (
    <div className={classNames('space-y-3', className)}>
      {!compact && <p className="text-sm font-medium text-ui-textPrimary">Repository Statistics:</p>}

      <div className={classNames('grid gap-3', compact ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-3')}>
        {totalFiles !== undefined && (
          <div className="flex items-center gap-2 text-ui-textPrimary">
      ...
      {languages && Object.keys(languages).length > 0 && (
        <div className={compact ? 'pt-1' : 'pt-2'}>
          <div className="flex items-center gap-2 text-ui-textPrimary mb-2">
